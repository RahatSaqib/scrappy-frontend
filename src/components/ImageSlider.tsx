...
export const ImageSlider = (props: any) => {
    ...
    const indicators = (index: any) => (<img className='indicators' style={{ ...indicatorsStyle }} src={`${imageUrl + images[index].file_key}`}></img>);
    return (
        <Zoom autoplay={false} indicators={indicators} scale={1.4}>
            {images?.map((image: any, index: number) => {
                return (
                    <div className="each-slide-effect" key={`image-${index}`} >
                        <img style={{ ...divStyle }} src={`${imageUrl + image.file_key}`}></img>
                    </div>
